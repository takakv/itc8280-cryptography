...
\begin{task}{Exploring certificates}
  ...
  To print the full certificate in a human readable form, we can use:
  \begin{cmdblock}
openssl x509 -in ee.pem -noout -text
  \end{cmdblock}
  where \texttt{-noout} is necessary to avoid outputting the certificate's PEM, which the tool does by default.
  ...
  \begin{itemize}
    ...
    \begin{tcolorbox}[title={Abuse of terminology}]
      ...
      E.g. I might say \enquote{this public key may only be used for X} instead of \enquote{the certificate restricts this key's usage to X}.
    \end{tcolorbox}
    \begin{itemize}
      ...
      \texttt{ee.pem} is an end-entity certificate, and thus this capability is not set.
      ...
      \begin{itemize}
        ...
        A CA must have it, an end-entity must not have it.
    ...
    \begin{itemize}
      \item The Subject Key Identifier identifies a particular public key and is\footnotemark{} derived from the public key.
      \footnotetext{While it should be derived from the public key, it is not required by \href{https://datatracker.ietf.org/doc/html/rfc5280\#section-4.2.1.2}{RFC 5280}.}
      ...
      \item Notice that the AKI of \texttt{root.pem} is the SKI of \texttt{root.pem} and that the Issuer and Subject are also the same.
