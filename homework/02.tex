...
\begin{itemize}
  ...
  \end{itemize}
  \item Grace clause submission deadline: 22.03.2026 23:59 EET.
  \begin{itemize}
...
bruteforce(seed: bytes, difficulty: int) -> tuple[int, bytes]  
  ...
  starts with at least 25 zero-bits (the challenge \enquote{difficulty}).
  Use big-endian encoding for the nonce.
  That is, you try for nonces:
  \begin{enumerate}
  ...
  For example, for the seed \texttt{takraa}, the smallest integer nonce that produces the desired hash is $23742492$.
  Indeed, the hex value of this integer is \texttt{0x016a481c}, and
   \begin{Verbatim}
SHA512/256(b"takraa\x01\x6a\x48\x1c")
  \end{Verbatim}
  yields the hash value
  \begin{Verbatim}
0000005221e3e4d844fa3ec03d4eea2e1c0d595623fa6a4552acc5caa42e5647
  \end{Verbatim}
...
\begin{task}{Proof of inclusion}
  Task template file: \texttt{merkle\_prove\_template.py}
...
\end{itemize}
  ...
  \begin{itemize}
    \item Manually compute the necessary intermediate hashes and construct the \texttt{proof.json}.
    Explain what you did and how in your report.
    You will not lose points for this approach.
...
\begin{task}{Extending this assignment}
  Task template file: \texttt{forger\_template.py}

  Consider an authentication scheme over a plaintext channel where, to log into a system, a user:
  \begin{enumerate}
    \item Generates a 128-bit nonce uniformly at random
    \item MACs the nonce with a key known to users and the server
    \item Submits the (nonce, tag) pair to the server
  \end{enumerate}

  The server first checks whether the nonce has previously been used.
  If yes, the server rejects the login, otherwise it stores the nonce in its database.
  If the nonce was fresh, the server verifies the MAC on the nonce.
  If validation succeeds, the user is granted access.

  Implement the function
  \begin{Verbatim}
adversary(message: bytes, tag: bytes) -> tuple[bytes, bytes]
  \end{Verbatim}

  The function must implement the EUF-KMA adversary for the naive MAC $t \gets \mathsf{SHA256}(k||m)$ with 128-bit keys.
  The adversary does not have access to the MAC key or to a MAC oracle, which is why we consider KMA and not CMA.

  The function must return a valid (and fresh) (message, tag) pair.
\end{task}
