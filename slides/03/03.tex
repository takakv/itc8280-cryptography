...
\begin{frame}{Symmetric encryption recap}
  ...
  \begin{itemize}[<+(1)->]
    \item Do not reuse IVs/nonces
    
    \vspace*{0.5em}

    \item AES-GCM should typically be the first choice
    \begin{itemize}
      \item Key sizes: 128, 192, 256 bits, with AES-256 being quantum safe
      \item 96-bit IV (standard), longer IVs may be supported
      \item NB! Alternatives may be necessary (e.g. data limits)
    \end{itemize}

    \vspace*{0.5em}

    \item ChaCha20-Poly1305 otherwise
    \begin{itemize}[<+(1)->]
      \item e.g. systems without AES hardware support
      \item 256-bit key: quantum safe
      \item 96-bit nonce (192-bit nonce for XChaCha20-Poly1305)
      \item Caveat: not standardised by NIST (not FIPS-compliant)
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{IND-CCA2: adaptive}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}[
      ...
      \draw[divider,visible on=<3->] (-1, -1) -- (9, -1)
        node[above=2pt, anchor=west, fill=bg, inner sep=2pt] {};
      \node[phaselabel,visible on=<2->] at (\phasex, -0.5) {Inherit: setup + phase 1};
      ...
      \draw[divider,visible on=<5->] (-1, -4.5) -- (9, -4.5)
        node[below=2pt, anchor=west, fill=bg, inner sep=2pt] {};
      \node[phaselabel,visible on=<4->] at (\phasex, -3.75) {Phase 2 (adaptive)};
