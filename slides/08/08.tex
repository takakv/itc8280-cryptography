...
\begin{frame}{Transitive trust}
  ...
  \begin{itemize}[<+(1)->]
    \item Alice and Carol are friends
    \item Bob and Carol are friends
    \item Alice does not know Bob
  ...
  \pause
  Carol gives Bob her signature $\sigma_{\PK_B}$ on the message
  \begin{center}
...
\begin{frame}{Transitive trust}
  \begin{figure}
    ...
    \begin{tikzpicture}
      ...
      \draw[<-, thick] (alice.east) -- node[above] {$(m, \sigma),\ \bigl(C_B, \sigma_{\PK_B}\bigr)$} (bob.west);
      ...
      \draw[->,dashed,thick] (bob-right) -- node[above] {$C_B,\ \sigma_{\PK_B}$} (bob-right -| bob.north) -- (bob.north);
    \end{tikzpicture}
  \end{figure}
