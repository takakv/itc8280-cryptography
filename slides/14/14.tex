...
\begin{frame}{Two paradigms}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item Schemes based on verifiable shuffle (cryptography mixing)
  \end{itemize}
  ...
  \begin{itemize}[<+(1)->]
    \item Ballot secrecy: a voter's choice should remain secret
...
\begin{frame}{Privacy of aggregation}
  By decrypting the aggregated result, we decouple identities from the result.
  \begin{itemize}[<+(1)->]
...
\begin{frame}{Mistrusting the organiser}
  How can we prevent the organiser from breaching vote secrecy?
  \begin{itemize}[<+(1)->]
    \item Secret sharing
    \begin{itemize}
      \item Verifiable variants preferred
    \end{itemize}
    \item MPC / distributed cryptography for decryption
    \begin{itemize}
      \item Not always possible due to technical and/or performance constraints
    \end{itemize}
    \item Share generation and decryption ceremonies required
    \begin{itemize}
      \item Electronic support (e.g. JavaCards) should be sealed after generation
      \item Electronic support should be destroyed after tally and audit
    \end{itemize}
  \end{itemize} 
\end{frame}
...
\begin{frame}{Internet voting in Estonia}
  \begin{itemize}[<+(1)->]
    ...
    \item No known/reported security incidents
    \item 2023: first year where $> 50\%$ of votes were i-votes
    \item Current system called \enquote{IVXV} introduced in 2017
    \item Continuous improvement of the system
    \item Verifiable shuffle rather than homomorphic tally due to many candidates
  \end{itemize}
\end{frame}
...
\begin{frame}{On mitigating coercion}
  ...
  \begin{itemize}[<+(1)->]
    \item Voters can vote multiple times online
    \begin{itemize}
...
\begin{frame}{Election keyholders}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item 7 shares for the NEC, 2 for the SEO
    \item Cards are sealed with a tamper evident tape and signed by members present
    \item More than 5 can present their shares during decryption, if they so wish
  \end{itemize}
